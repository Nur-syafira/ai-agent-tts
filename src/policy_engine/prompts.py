...
Промпты можно кастомизировать через config.yaml (секция prompts).
Если промпты не указаны в конфиге, используются дефолтные значения.
"""
...
# Дефолтный system prompt (можно переопределить через config.yaml)
DEFAULT_SYSTEM_PROMPT = """Ты — профессиональный администратор, который принимает звонки и обрабатывает запросы клиентов.
...
# Дефолтная роль агента (можно переопределить через config.yaml)
DEFAULT_AGENT_ROLE = "администратор"
...
def get_system_prompt(custom_prompt: Optional[str] = None) -> str:
    ...
    Args:
        custom_prompt: Кастомный промпт из конфига (опционально)
...
def get_agent_role(custom_role: Optional[str] = None) -> str:
    """
    Получить роль агента.
    
    Args:
        custom_role: Кастомная роль из конфига (опционально)
        
    Returns:
        Роль агента для использования в промптах
    """
    return custom_role if custom_role else DEFAULT_AGENT_ROLE
...
def get_response_generation_prompt(
    ...
    Args:
        ...
        agent_role: Роль агента (опционально, по умолчанию из конфига)
    ...
    role = get_agent_role(agent_role)
    
    return f"""Ты {role}. Сгенерируй КОРОТКУЮ и естественную реплику для текущего состояния диалога.
